...
int main(int argc,  char *argv[ ]) {
   ...
   int i, k;                      /* number of this process (starting with 1)   */
   ...
   for (i = 1; i < nprocs;  i++) {         /* create the remaining processes */
	...
	pids[0] = next_id;
	for(k = 0; k < nprocs; k++){
		ret = write(STDOUT_FILENO, &next_id, sizeof(long));
		ret = read(STDIN_FILENO, &next_id, sizeof(long));
		pids[k] = next_id;
	}
	ret = wait(&status);
	for(k = 0; k < nprocs; k++){
		fprintf(stderr, "this is process %ld with PPID %ld and process %ld is next in line\n", (long)getpid(), (long)getppid(), pids[k]);
	}
